...
<div class="draft"><b>EVIDENCE STATUS MATTERS.</b> Missing measurements and model exports remain explicitly unavailable.</div>
...
<section class="section" id="screening"><div class="kicker">04 Molecular safety and environmental screening</div><h2>External model outputs are not yet available</h2><p class="lead">ADMETlab, ProTox, CompTox and ECOSAR are cache-first inputs. The pipeline will generate toxicity consensus, uncertainty and environmental-fate figures only after unmodified exports are supplied.</p>{model_table}<div class="note warning"><b>Interpretation boundary:</b> computational toxicology is screening evidence. It cannot establish safety or replace a containment experiment.</div><figure class="figure"><img src="figures/framework/evidence_provenance_matrix.svg" alt="Evidence provenance matrix"><figcaption>Every evidence stream is labeled by source and availability. Missing values are not converted to zeros.</figcaption></figure></section>
...
def export_wiki() -> Path:
    ...
    for directory in ["results/tables", "docs/methodology", "docs/provenance", "docs/limitations"]:
        shutil.copytree(ROOT / directory, destination / directory, dirs_exist_ok=True)
