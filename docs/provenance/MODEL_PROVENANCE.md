...
The full cache-first pipeline and 14 tests passed on Matvision. Raw inputs remain byte-identical to Git HEAD. Predictions were reused, not recomputed. Actual environment versions are recorded in results/summaries/matvision_reproduction.json; the environment differs from the original inference environment.
